...
insurance_input = st.selectbox("Select Insurance:", options=[opt for opt in [""] + list(df['Ins Full Name'].unique()) if opt.strip()], format_func=lambda x: x if x else "Type to search..."), format_func=lambda x: x if x else "Type to search...")
